...
def train(batch_size, prefix):
   ...
   # net = sym.make_l1_sym(scale, 'sum', data_frames, flow_frames, method=method, tan=tan)
    # 그럼 이 부분은 depth image 부분이고
    train_data = data.Mov3dStack('data/lmdb', (384, 160), batch_size, scale, output_depth=False,
                                 data_frames=data_frames, flow_frames=flow_frames, stride=stride, no_left0=no_left0,
                                 right_whiten=right_whiten)
    #영상 받아오는 부분 left imgae
    ...
    init = mx.init.Load(vgg16, mx.init.Uniform(0.01), True) # vgg로드시켜서 conv 적용시키는듯
    init = mx.init.Mixed(['deconv_pred.*weight', '.*'], [bilinear, init])
    # 내 생각엔 이 부분이 거리 계산해서 오른쪽 이미지 만들어 주는 곳 bilinear 이 뎁스를 오른쪽으로 옮기고 이 부분은 원래 이미지 변환
    #

    model = mx.model.FeedForward(
        ctx=[mx.gpu(0)],
        symbol='depth image 넘어온 변수   net',
        num_epoch=100,
        learning_rate=0.002,
